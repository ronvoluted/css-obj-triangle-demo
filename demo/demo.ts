...
type RenderMeshOptions = {
  ...
  pose?: MeshPose;
  doubleSided?: boolean;
  colorForFace?: (faceIndex: number, normal: Vec3) => string;
};
...
type CSSOBJ3DAPIs = {
  ...
  prepareOBJ: typeof prepareOBJ;
  triangleMatrix3d: typeof triangleMatrix3d;
  triangleStyleFromVertices: typeof triangleStyleFromVertices;
  meshTransformCss: typeof meshTransformCss;
  applyMeshPose: typeof applyMeshPose;
  renderOBJMesh: typeof renderOBJMesh;
};
...
function resolveOBJIndex(index: number, vertexCount: number): number {
...
function parseOBJ(objText: string): ParsedOBJ {
  ...
  for (const rawLine of objText.split(/\r?\n/)) {
    ...
    if (kind === "f" && parts.length >= 4) {
      ...
      if (vertexRefs.length < 3) continue;
...
function getBounds(vertices: Vec3[]): { min: Vec3; max: Vec3; size: Vec3; center: Vec3 } {
...
function prepareOBJ(parsed: ParsedOBJ, options: ImportOptions = {}): ParsedOBJ {
  ...
  const faces = shouldReverse
    ? parsed.faces.map(([a, b, c]) => [a, c, b] as Face)
    : parsed.faces.slice();
...
function triangleStyleFromVertices(
  a: Vec3,
  b: Vec3,
  c: Vec3,
  color = "crimson",
...
function meshTransformCss(pose: MeshPose): string {
  return [
...
function applyMeshPose(meshEl: HTMLElement, pose: MeshPose): void {
  meshEl.style.transform = meshTransformCss(pose);
}

function defaultColorForFace(faceIndex: number, normal: Vec3): string {
...
function renderOBJMesh(meshEl: HTMLElement, objText: string, options: RenderMeshOptions = {}): void {
...
(window as Window & { CSSOBJ3D?: CSSOBJ3DAPIs }).CSSOBJ3D = {
  parseOBJ,
  prepareOBJ,
  triangleMatrix3d,
